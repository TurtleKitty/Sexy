...
(define (check-sexy-use code)
    (define usage '(use <name> <symbol-or-string-path> <arg> ...))
    (if (< (length code) 3)
        (syntax-error code "use: too few arguments." usage)
        (let ((name (cadr code)) (uri (caddr code)))
            (if (not (symbol? name))
                (syntax-error code "use: first argument must be a symbol." usage)
                (if (not (or (symbol? uri) (string? uri)))
                    (syntax-error code "use: second argument must be a symbol or string." usage)
                    #t)))))
...
(define (check-sexy-syntax-export code)
    ...
    (let ((rest (cdr code)))
        (if (not (pair? rest))
            (syntax-error code "syntax: must have at least 1 argument." usage)
